...
from loader.augs import (BinarizeLabel, GaussianBlur, GenInstanceDistance,
                         ...
                         GenInstanceContourMap, GenInstanceDG)
...
#             'Spindle'      : 4,
        ...
        data_code_dict = {
            'unet'     : '512x512_356x356',
            'hcnet'     : '512x512_256x256_dist',
            'dist'     : '536x536_84x84',
            'fcn8'     : '512x512_256x256',
            'dcan'     : '512x512_256x256',
            'segnet'   : '512x512_256x256',
            'micronet' : '504x504_252x252', 
            'np_hv'  : '400x400_160x160',
            'np_dg'  : '400x400_160x160',
        ...
        self.train_dir = ['/home1/gzy/NucleiSegmentation/High_CCRCC/Train/%s/'  % data_code_dict[self.model_type]]
        print(self.train_dir)
        self.valid_dir = ['/home1/gzy/NucleiSegmentation/High_CCRCC/Valid/%s/' % data_code_dict[self.model_type]]
        ...
        self.inf_model_path  = self.save_dir + '/01/model-40800.index'
    ...
    def get_model(self):
        ...
        elif self.model_type == 'np_dg':
            model_constructor = importlib.import_module('model.graph')
            model_constructor = model_constructor.Model_NP_DG
    ...
    def get_train_augmentors(self, input_shape, output_shape, view=False):
        ...
        label_augs = []
        if self.model_type == 'unet' or self.model_type == 'micronet' or self.model_type=='hcnet':
        ...
        if self.model_type == 'np_dg':
            label_augs = [GenInstanceDG(crop_shape=output_shape)]
#             label_augs = []
