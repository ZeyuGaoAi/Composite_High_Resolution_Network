...
from model.utils import crop_op
...
class StatCollector(Inferencer, Config):
    ...
    def _after_inference(self):
        ...
        elif (self.model_type == 'np_dg' or self.model_type == 'hcnet') and self.regression:
            pred_dg = pred_inst[...,-1]
            true_dg = true_inst[...,-1]/255
            error = true_dg - pred_dg
#             mse = np.sum(error * error) / nr_pixels
            mae = np.sum(np.abs(error)) / nr_pixels
#             mcc = np.sum(np.exp(-np.square(error)/(0.2*0.2*2))) / nr_pixels
            ...
            pred_pg = pred_dg[...,-2]
            true_pg = true_dg[...,-2]
            for i in range(true_ng.shape[0]):
                ssim_ng.append(ssim_metric(true_ng[i], pred_ng[i], data_range=1.0, win_size=11, K1=0.01, K2=0.03, gaussian_weights=1.5))
                ssim_pg.append(ssim_metric(true_pg[i], pred_pg[i], data_range=1.0, win_size=11, K1=0.01, K2=0.03, gaussian_weights=1.5))
            
            ssim = np.mean(ssim_ng) + np.mean(ssim_pg)
            
#             stat_dict[self.prefix + '_mse'] = mse
            stat_dict[self.prefix + '_mae'] = mae
            stat_dict[self.prefix + '_ssim'] = ssim
#             stat_dict[self.prefix + '_mcc'] = mcc

        # classification statistic
        if self.model_type != 'dist':# and self.type_nuclei:
            pred_np = pred_inst[...,0]
            true_np = true_inst[...,0]
...
class Trainer(Config):   
    ...
    def view_dataset(self, mode='train'):
        ...
        return
    ####
    def run_once(self, opt, nr_gpus, sess_init=None, save_dir=None):
        ####
...
#                     continue
                ...
                self.run_once(opt, nr_gpus, sess_init=init_weights, save_dir=log_dir)
                prev_log_dir = log_dir
        else:
            random.seed(self.seed)
            np.random.seed(self.seed)
            ...
            self.run_once(opt, nr_gpus, sess_init=init_weights, save_dir=self.save_dir)
